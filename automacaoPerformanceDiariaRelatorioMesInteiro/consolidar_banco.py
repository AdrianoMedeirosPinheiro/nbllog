...
def processar_arquivo_excel(caminho_arquivo, tabela_destino, engine):
    ...
    try:
        ...
        # 3) Exclui todos os registros do mês atual antes de inserir novos
        with engine.connect() as conn:
            delete_query = text(f'''
                DELETE FROM {tabela_destino}
                WHERE DATE_TRUNC('month', "DATA_DO_RELATORIO") = DATE_TRUNC('month', CURRENT_DATE)
            ''')
            conn.execute(delete_query)
            conn.commit()
        print("Registros do mês atual excluídos com sucesso.")
