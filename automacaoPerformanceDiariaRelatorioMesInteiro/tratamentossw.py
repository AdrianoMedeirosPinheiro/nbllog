...
def ler_unico_arquivo_ssw(caminho_arquivo):
    """
    Lê um arquivo .sswweb específico e retorna:
      - DataFrame com as colunas definidas
      - Caminho do arquivo
    """
    try:
        ...
        print(f"[ERRO] Problema ao ler arquivo {caminho_arquivo}: {e}")
        return pd.DataFrame(), None

def ler_linhas_ignoradas(caminho_arquivo):
    """
    Lê as primeiras 5 linhas do arquivo e extrai datas e horários.
    Retorna um DataFrame com as linhas ignoradas e as variáveis:
      DATA_DO_RELATORIO, HORA_DO_RELATORIO, MES_DO_RELATORIO, DATA_COMPETENCIA
    """
    try:
        with open(caminho_arquivo, 'r', encoding='latin1') as file:
        ...
        print(f"[ERRO] Problema ao ler linhas iniciais de {caminho_arquivo}: {e}")
        return pd.DataFrame(), None, None, None, None

def processar_unico_arquivo_ssw(caminho_arquivo):
    """
    Executa o processamento para UM arquivo:
      ...
      6) Inclui colunas de datas/hora lidas das linhas ignoradas.
    """
    df, _ = ler_unico_arquivo_ssw(caminho_arquivo)
    if df.empty:
        # Retorna vazio se não leu nada
        return pd.DataFrame()

    # Lê linhas ignoradas (cabeçalho)
    ...
    # Regex para identificar CTRC (ex: 'ABC 123456-7')
...
def processar_varios_arquivos_ssw(diretorio):
    """
    Percorre o diretório procurando TODOS os arquivos .sswweb,
    processa cada um e concatena num único DataFrame.
    """
    arquivos_ssw = [
        os.path.join(diretorio, f)
        for f in os.listdir(diretorio)
        if f.lower().endswith('.sswweb')
    ]
    ...
    for arquivo in arquivos_ssw:
        ...
        df_processado = processar_unico_arquivo_ssw(arquivo)
    ...
    if lista_dfs:
        ...
        # Se nenhum arquivo retornou dados úteis
        return pd.DataFrame()
