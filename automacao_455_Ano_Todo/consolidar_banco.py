...
def criar_tabela_se_nao_existir(engine, tabela_destino, caminho_arquivo, ano_especifico, chunk_size=50000):
    if inspect(engine).has_table(tabela_destino):
        print(f"A tabela {tabela_destino} já existe. Excluindo registros do ano {ano_especifico} nos últimos 12 meses.")
        with engine.connect() as conn:
            trans = conn.begin()
            try:
                # Define o intervalo de exclusão para os últimos 12 meses do ano específico
                inicio_exclusao = f'{ano_especifico}-01-01'
                fim_exclusao = f'{ano_especifico}-12-31'
                
                conn.execute(text(f"""
                    DELETE FROM {tabela_destino}
                    WHERE data_de_emissao BETWEEN '{inicio_exclusao}' AND '{fim_exclusao}'
                """))
                trans.commit()
                print(f"Exclusão de registros do ano {ano_especifico} concluída com sucesso.")
...
def consolidar_arquivos_sswweb_para_postgres(diretorio, tabela_destino, engine, ano_especifico, chunk_size=50000):
    arquivos_sswweb = [
    ...
    if arquivos_sswweb:
        # Seleciona o primeiro arquivo como referência para criar a tabela
        caminho_arquivo_referencia = arquivos_sswweb[0]
        criar_tabela_se_nao_existir(engine, tabela_destino, caminho_arquivo_referencia, ano_especifico, chunk_size)
