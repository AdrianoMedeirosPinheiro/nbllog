...
def iniciar_download(new_page, filtered_rows, download_directory, loading_selector="#procimg"):
    ...
    # Verifica se foram realizados 12 downloads
    downloaded_files_count = len([f for f in os.listdir(download_directory) if os.path.isfile(os.path.join(download_directory, f))])
    if downloaded_files_count == 12:
        print(f"Todos os 12 arquivos foram baixados com sucesso no diretório: {download_directory}")
    else:
        print(f"Atenção: apenas {downloaded_files_count} de 12 arquivos foram baixados.")
