...
with sync_playwright() as p:
    ...
    time.sleep(3)
    popup_page.wait_for_selector('#nro_lancto')

    gerar_relatorio.gerar_relatorios(popup_page, config_baixar_todos=False)

    with popup_page.expect_popup() as popup_info:
        popup_page.click('#link_fila')
    ...
    filtered_rows = ultimas_linhas.filtrar_ultimas_linhas(rows, criterios_opcao, criterios_usuario, config_filtrar_todos=False)
    filtered_rows_atualizado = verificar_concluida.verificar_situacao_concluida(new_page, criterios_opcao, criterios_usuario, config_verificar_todos=False)
    download_directory = "/nbllog/automacao_477/base"
    limpar_diretorio.limpar_diretorio(download_directory)
    time.sleep(2)
    download.iniciar_download(new_page, filtered_rows, download_directory, config_baixar_todos=False)
    time.sleep(2)
    ...
    diretorio = "/nbllog/automacao_477/base"  # Caminho para o diretório dos arquivos .sswweb
    tabela_destino = 'relatorios477'  # Nome da tabela no PostgreSQL
