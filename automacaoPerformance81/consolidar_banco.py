...
def criar_tabela_se_nao_existir(engine, tabela_destino, df_template):
    ...
    if inspect(engine).has_table(tabela_destino):
        ...
        # Mapear tipos de colunas (ignorando ARQUIVO_ORIGEM caso exista no df_template)
        if "ARQUIVO_ORIGEM" in df_template.columns:
            df_template = df_template.drop(columns=["ARQUIVO_ORIGEM"])
...
def inserir_dados_usando_copy(df, tabela_destino, engine):
    ...
    # Ignorar a coluna ARQUIVO_ORIGEM, caso exista
    if "ARQUIVO_ORIGEM" in df.columns:
        df = df.drop(columns=["ARQUIVO_ORIGEM"])
...
def processar_arquivo_excel(caminho_arquivo, tabela_destino, engine):
    ...
    try:
        ...
        # Se existir "ARQUIVO_ORIGEM", vamos removê-la para não inserir no banco.
        if "ARQUIVO_ORIGEM" in df.columns:
            df.drop(columns=["ARQUIVO_ORIGEM"], inplace=True)
            print("Coluna 'ARQUIVO_ORIGEM' removida antes da inserção.")
